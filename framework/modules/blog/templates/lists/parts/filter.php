<?php
$filter = esc_attr(chandelier_elated_options()->getOptionValue('masonry_filter'));

if($filter == 'yes'){
	$page_category = get_post_meta(chandelier_elated_get_page_id(), "eltd_blog_category_meta", true);
	if(is_category()){
	...
	if (count($categories) > 0) {	?>
		<div class="eltd-filter-blog-holder">
			<div class="eltd-filter-blog">
				<ul>
					<li class="eltd-filter eltd-active" data-filter="*"><span><?php esc_html_e('All', 'chandelier'); ?></span></li>
