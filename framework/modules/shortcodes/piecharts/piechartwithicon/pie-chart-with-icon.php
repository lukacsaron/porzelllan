<?php
namespace Chandelier\Modules\PieCharts\PieChartWithIcon;
...
class PieChartWithIcon implements ShortcodeInterface {
			...
			'params' => array_merge(
				...
				array(
					array(
						...
						'dependency' => Array('element' => 'icon_pack', 'value' => chandelier_elated_icon_collections()->getIconCollectionsKeys()),
						'group' => 'Design Options',
					),
					array(
						'type' => 'textfield',
	...
	public function render($atts, $content = null)
	{
		...
		$html = chandelier_elated_get_shortcode_module_template_part('templates/pie-chart-with-icon', 'piecharts/piechartwithicon', '', $params);
	...
	private function getPieChartData($params) {
		...
		if( $params['percentage_color'] !== '' ) {
			$pieChartData['data-percentage-color'] = $params['percentage_color'];
		}
		if( $params['inactive_color'] !== '' ) {
			$pieChartData['data-inactive-color'] = $params['inactive_color'];
		}
	...
	private function getPieChartIcon($params) {
		...
		$pie_chart_icon = chandelier_elated_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
