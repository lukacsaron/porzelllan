...
if(!function_exists('chandelier_elated_get_title')) {
    /**
     * Loads title area HTML
     */
    function chandelier_elated_get_title() {
        $id = chandelier_elated_get_page_id();

        extract(chandelier_elated_title_area_height());
        extract(chandelier_elated_title_area_background());


        //check if title area is visible on page first, then in the options
        if(get_post_meta($id, "eltd_show_title_area_meta", true) !== ""){
            $show_title_area = get_post_meta($id, "eltd_show_title_area_meta", true) == 'yes' ? true : false;
        }else {
            $show_title_area = chandelier_elated_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }

        //check for title type on page first, then in options
        if(get_post_meta($id, "eltd_title_area_type_meta", true) !== ""){
            $type = get_post_meta($id, "eltd_title_area_type_meta", true);
        }else {
            $type = chandelier_elated_options()->getOptionValue('title_area_type');
        }

        //check if breadcrumbs are enabled on page first, then in options
        if(get_post_meta($id, "eltd_title_area_enable_breadcrumbs_meta", true) !== ""){
            $enable_breadcrumbs = get_post_meta($id, "eltd_title_area_enable_breadcrumbs_meta", true) == 'yes' ? true : false;
        }else {
            $enable_breadcrumbs = chandelier_elated_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }

        //check if title color is set on page
        $title_color = '';
        if(get_post_meta($id, "eltd_title_text_color_meta", true) !== ""){
            $title_color = 'color:'.get_post_meta($id, "eltd_title_text_color_meta", true).';';
        }

        //check if subtitle color is set on page
        $subtitle_color = '';
        if(get_post_meta($id, "eltd_subtitle_color_meta", true) !== ""){
            $subtitle_color = 'color:'.get_post_meta($id, "eltd_subtitle_color_meta", true).';';
        }
        ...
        $parameters = apply_filters('chandelier_elated_title_area_height_params', $parameters);

        chandelier_elated_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('chandelier_elated_get_title_text')) {
    /**
     * Function that returns current page title text. Defines chandelier_elated_title_text filter
     ...
     */
    function chandelier_elated_get_title_text() {
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'chandelier') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search', 'chandelier');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
            if(chandelier_elated_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = chandelier_elated_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'chandelier');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(chandelier_elated_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(chandelier_elated_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','chandelier');
        }
        ...
        $title = apply_filters('chandelier_elated_title_text', $title);

        return $title;
    }
}

if(!function_exists('chandelier_elated_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see chandelier_elated_get_title_text()
     */
    function chandelier_elated_title_text() {
        echo chandelier_elated_get_title_text();
    }
}

if(!function_exists('chandelier_elated_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function chandelier_elated_subtitle_text() {
        $id 	= get_queried_object_id();
        ...
        if(get_post_meta($id, "eltd_title_area_subtitle_meta", true) != ""){
            $subtitle = wp_kses_post(get_post_meta($id, "eltd_title_area_subtitle_meta", true));
        }
...
if(!function_exists('chandelier_elated_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see chandelier_elated_is_product_category()
     * @see get_search_query()
     */
    function chandelier_elated_custom_breadcrumbs() {
        ...
        $delimiter = '<span class="eltd-delimiter" '.chandelier_elated_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltd-current" '.chandelier_elated_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.chandelier_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.chandelier_elated_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.chandelier_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.chandelier_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || chandelier_elated_is_product_category()) {
                ...
                $output .= '<a '.chandelier_elated_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.chandelier_elated_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.chandelier_elated_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.chandelier_elated_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'chandelier') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('chandelier_elated_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function chandelier_elated_get_title_area_height() {
		...
		if($title_type == "standard"){
			$title_height = 345; // default title height without header height
		}elseif($title_type == "light"){
			$title_height = 180;
		}     
        elseif($title_type == "breadcrumb") {
            $title_height = 120;
        }

        if(get_post_meta($id, "eltd_title_area_height_meta", true) != '') {
            $title_height = get_post_meta($id, 'eltd_title_area_height_meta', true);
        } elseif(chandelier_elated_options()->getOptionValue('title_area_height') !== '') {
            $title_height = chandelier_elated_options()->getOptionValue('title_area_height');
        }

        return apply_filters('chandelier_elated_title_area_height', $title_height);
    }
}

if(!function_exists('chandelier_elated_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function chandelier_elated_get_title_content_padding() {
	    $title_content_padding = apply_filters( 'chandelier_elated_title_content_padding', 0 );
...
if(!function_exists('chandelier_elated_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function chandelier_elated_title_area_height() {
        $id = chandelier_elated_get_page_id();
        $title_height_and_padding = array();
        $title_height          = chandelier_elated_get_title_area_height();
        $header_height_padding = chandelier_elated_get_title_content_padding();
        ...
        if(get_post_meta($id, "eltd_title_area_vertial_alignment_meta", true) !== '') {
            $title_vertical_alignment = get_post_meta($id, "eltd_title_area_vertial_alignment_meta", true);
        }else{
            $title_vertical_alignment = chandelier_elated_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('chandelier_elated_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function chandelier_elated_title_area_background() {
        $id = chandelier_elated_get_page_id();
        $show_title_img = true;
        ...
        if(get_post_meta($id, "eltd_title_area_background_color_meta", true) != ""){
            ...
            $background_color = chandelier_elated_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(get_post_meta($id, "eltd_title_area_background_image_meta", true) != ""){
            ...
            $background_image = chandelier_elated_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = chandelier_elated_get_image_dimensions($background_image);
