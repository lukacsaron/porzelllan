...
class ChandelierInstagramWidget extends ChandelierWidget {
    protected $params;
    public function __construct() {
        parent::__construct(
            'eltd_instagram_widget',
            'Elated Instagram Widget',
            array( 'description' => esc_html__( 'Display instagram images', 'chandelier' ) )
        );
    ...
    public function widget($args, $instance) {
        ...
        if(is_array($images_array) && count($images_array)) { ?>
			<h6 class="eltd-widget-title">
				<?php echo esc_attr($title)?></h6>
            <ul class="eltd-instagram-feed clearfix eltd-col-<?php echo esc_attr($number_of_cols); ?>">
            <?php
            foreach ($images_array as $image) { ?>
                <li>
                    <a href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
                        <?php echo chandelier_elated_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
                    </a>
                </li>
            <?php } ?>
            </ul>
