(function($){
			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=chandelier_elated_gallery_upload_get_images&ids=" + ids,
	...
	function eltdInitAjaxForm() {
		...
		$(document).delegate(".eltd_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "chandelier_elated_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
...
//            }
	...
	function eltdShowHidePostFormats(){
		$('input[name="post_format"]').each(function(){
			$('#eltd-meta-box-'+ $(this).attr("id")+'-meta').hide();
		});
		var $selected = $("input[name='post_format']:checked").attr("id");
		$('#eltd-meta-box-'+ $selected+'-meta').fadeIn();
