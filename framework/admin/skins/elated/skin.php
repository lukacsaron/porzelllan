...
class ElatedSkin extends ChandelierSkinAbstract {
    /**
     * Skin constructor. Hooks to chandelier_elated_admin_scripts_init and chandelier_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('chandelier_elated_admin_scripts_init', array($this, 'registerStyles'));
        add_action('chandelier_elated_admin_scripts_init', array($this, 'registerScripts'));

        add_action('chandelier_elated_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('chandelier_elated_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('chandelier_elated_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('chandelier_elated_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            chandelier_elated_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            chandelier_elated_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = chandelier_elated_get_admin_tab();
        $active_page = $chandelier_elated_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param ChandelierAdminPage $page current page to load
     *
     * @see ElatedSkinAbstract::loadTemplatePart()
     */
